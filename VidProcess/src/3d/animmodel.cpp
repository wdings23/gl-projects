...
static int readNormals( TiXmlNode* pRoot,
                    ...
                    const char* szFileName )
{
    ...
    if( bLoaded )
    {
        TiXmlNode* pRoot = doc.FirstChild( "animmodel" );
...
//OUTPUT( "number of vert=%d total vert=%d\n", iNumVerts, iTotalVerts );
        ...
        pAnimModel->miNumNormals = iTotalNormals;
...
//OUTPUT( "total uv = %d\n", iTotalUV );
        ...
        while( pMeshNode )
        {
            ...
            int iNumNorms = readNormals( pMeshNode, iCurrTotalNormals, pAnimModel->maNorm, pAnimModel );
...
//OUTPUT( "number of vert=%d total vert=%d\n", iNumVerts, iCurrTotalVerts );
            ...
            WTFASSERT2( iCurrTotalNormals <= iTotalNormals, "too many normals" );
        ...
        for( int i = 0; i < pAnimModel->miNumVerts * MAX_JOINT_INFLUENCES; i++ )
        {
			...
			WTFASSERT2( bFound, "didn't find joint %s", pJoint->mszName );
...
{
    int iNumNormals = 0;
    TiXmlNode* pNormalNode = pRoot->FirstChild( "data" )->FirstChild( "normal" );
    while( pNormalNode )
    {
        TiXmlElement* pElement = pNormalNode->ToElement();
        const char* szX = pElement->Attribute( "x" );
        const char* szY = pElement->Attribute( "y" );
        const char* szZ = pElement->Attribute( "z" );
        
		int iIndex = iNumTotalNormals + iNumNormals;
        WTFASSERT2( iIndex < pAnimModel->miNumNormals, "too many normals" );

		tVector4* pNormal = &aNormals[iIndex];

		if( szX == NULL || szY == NULL || szZ == NULL )
		{
			const char* szNormal = pNormalNode->FirstChild()->Value();
			parseVector( pNormal, szNormal );
		}
		else
		{
			pNormal->fX = (float)atof( szX );
			pNormal->fY = (float)atof( szY );
			pNormal->fZ = (float)atof( szZ );
			pNormal->fW = 1.0f;
		}
                     ...
                     tAnimModel const* pAnimModel )
{
    ...
    while( pFaceNode )
    {
        ...
        while( pVNode )
        {
            ...
            int iV = (int)atoi( pElement->Attribute( "id" ) );
            int iUV = (int)atoi( pElement->Attribute( "uv" ) );
            int iNorm = (int)atoi( pElement->Attribute( "normal" ) );
...
//OUTPUT( "%d joint = %s blend: %f\n", iIndex, apJoints[iIndex]->mszName, afBlendVal[iIndex] );
        ...
        WTFASSERT2( ( pJoint || !strcmp( szJointName, "none" ) ), "can't find joint %s", szJointName );
...
static void consolidateNormals( tAnimModel* pAnimModel )
{
    ...
    int iNumNormals = pAnimModel->miNumNormals;

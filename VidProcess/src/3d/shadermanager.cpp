    ...
    if( bLoaded )
    {
        ...
        while( pNode )
        {
            ...
            while( pChild )
            {
                ...
                else if( !strcmp( szProperty, "text" ) )
                {
                    const char* szText = pChild->FirstChild()->Value();
                    strncpy( pProgram->mszText, szText, sizeof( pProgram->mszText ) );
                }
...
**
*/
tShaderProgram const* CShaderManager::getShaderProgramWithShaderID( GLuint iShaderID )
{
    tShaderProgram const* pRet = NULL;
    for( int i = 0; i < miNumShaderPrograms; i++ )
    {
        if( maShaderPrograms[i].miID == iShaderID )
        {
            pRet = &maShaderPrograms[i];
            break;
        }
    }

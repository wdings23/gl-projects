...
//double fStart = getCurrTime();
...
#if defined( BATCH_WITH_DEPTH_TEST )
	for( unsigned int i = 0; i < pBatchManager->miNumBatchAlloc; i++ )
	{	
	...
	tModelBatch* pLastBatch = NULL;
	if( pBatchManager->miNumBatches == 0 )
    {
        pToInsert = NULL;
        bFound = false;
    }
    ...
    {
        pLastBatch = &pBatchManager->maBatches[pBatchManager->miNumBatches-1];
        if( pLastBatch->maiTextureID[0] == pInfo->maiTextureID[0] )
        {
            pToInsert = pLastBatch;
            bFound = true;
        }
    }

#endif // BATCH_WITH_DEPTH_TEST

//double fElapsed = getCurrTime() - fStart;
//gfTotalSearchTime += fElapsed;

	tMatrix44 xformWithAnimMatrix;
	Matrix44Multiply( &xformWithAnimMatrix, 
					  &pInfo->mpModelInstance->mXFormMat, 
					  &pInfo->mpModelInstance->mAnimMatrix );

	modelBatchInfo.mpModelBatch = NULL;
	modelBatchInfo.mpModelInstance = pInfo->mpModelInstance;
	modelBatchInfo.mpXFormMatrix = &xformWithAnimMatrix;
								  ...
								  tVector4 const* pCameraPosition )
{
	...
	for( ; glInitIter != pBatchManager->maNeedInitBatches.end(); ++glInitIter )
	{
		int iBatchIndex = *glInitIter;
		tModelBatch* pBatch = &pBatchManager->maBatches[iBatchIndex];
		modelBatchSetupNormalGL( pBatch, pShader );
	}

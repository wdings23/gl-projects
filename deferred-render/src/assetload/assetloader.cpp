...
static void loadModel( void* pData );
static void loadTexture( void* pData );
...
void processAssetOnMainThread( void )
{
	...
	for( ; it != gMainThreadQueue.end(); ++it )
	{
		...
		else if( pJobData->miType == ASSET_TEXTURE )
		{
			...
			tTexture* pTexture = CTextureManager::instance()->getTexture( szName );
            WTFASSERT2( pTexture, "can't load texture: %s", szName );
			++iIndex;
		}

...
*/
       ...
       // worker is busy
       WTFASSERT2( pJobData->miWorkerID >= 0 && pJobData->miWorkerID < 4, "invalid worker id" );
       *piRunning = 1;
...
void jobManagerAddJob( tJobManager* pJobManager, tJob* pJob )
{
    ...
    if( pJobManager->miNumJobs + 1 >= pJobManager->miQueueSize )
    {
        ...
        for( int i = 0; i <= iEnd; i++ )
        {
            ...
            WTFASSERT2( iPrevQueueSize + i < pJobManager->miQueueSize, "array out of bounds" );
...
bool jobManagerRunning( tJobManager* pJobManager )
{
    ...
    for( int i = 0; i < pJobManager->miNumWorkers; i++ )
    {
        if( pJobManager->maiRunning[i] )
        {
            bThreadRunning = true;

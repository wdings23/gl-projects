                        ...
                        CFactory<tMatrix44>* pMatrixFactory )
{
	...
	if( bLoaded )
	{
		TiXmlNode* pNode = doc.FirstChild( "model" )->FirstChild( "joints" );
		TiXmlNode* pChild = pNode->FirstChild( "properties" );
		...
		while( pChild )
		{
			...
			pChild = pChild->NextSibling( "properties" );
		}
		
		pAnimHierarchy->maJoints = pJointFactory->alloc( pAnimHierarchy->miNumJoints );
		pChild = pNode->FirstChild( "properties" );
		...
		while( pChild )
		{
			...
			else if( !strcmp( szVal, "data" ) )
			{
				...
				while( pDataNode )
				{
					...
					{
						TiXmlElement* pElement = pDataNode->ToElement();
						szParentName = pElement->Attribute( "parent1" );
					}
